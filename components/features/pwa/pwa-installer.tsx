...
import { Download, Monitor, Smartphone, Chrome, CheckCircle, Star, ShieldCheck } from "lucide-react"
...
function detectInstalledMode() {
  ...
  // iOS Safari
  const isNavigatorStandalone = "standalone" in navigator && (navigator as Navigator & { standalone?: boolean }).standalone === true
...
export function PWAInstaller() {
  ...
  useEffect(() => {
    ...
    const syncInstalledState = () => {
      const fromStorage = localStorage.getItem(PWA_INSTALLED_FLAG) === "1"
      const fromRuntime = detectInstalledMode()
      const installedNow = fromStorage || fromRuntime
  ...
  const handleInstall = async () => {
    if (!deferredPrompt) {
      setInstallError("Instalacja nie jest teraz dostępna. Użyj menu przeglądarki: Dodaj do ekranu głównego / Zainstaluj aplikację.")
      return
    }
    ...
    try {
      ...
      setInstallError("Nie udało się uruchomić instalacji z poziomu przycisku. Użyj opcji instalacji z menu przeglądarki.")
    } finally {
      setInstalling(false)
    }
  }
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogTrigger asChild>
        ...
        >
          {isInstalled ? <ShieldCheck className="h-3 w-3 text-primary" /> : <Download className="h-3 w-3" />}
          {isInstalled ? "JUŻ_ZAINSTALOWANA" : "INSTALL"}
        </Button>
      </DialogTrigger>
      <DialogContent className="max-w-2xl font-mono">
          ...
          <div className="flex items-center gap-3 border border-primary/50 bg-primary/10 p-4">
            ...
            <div>
              ...
              <p className="mt-2 text-[11px] text-muted-foreground">
                Otwieraj aplikację z ikony systemowej (Start/Pulpit/Home Screen), aby uruchamiać od razu wersję zainstalowaną.
              </p>
            </div>
          </div>
        ) : (
        ...
        <div className="mt-4 border-t border-border pt-4">
          {installError && (
            <div className="mb-3 border border-red-500/40 bg-red-500/10 p-2 text-xs text-red-300">
              {installError}
            </div>
          )}
